...
## Compatibilidade com FluidNC recentes (WebSocket)
As versões recentes do FluidNC podem usar porta e path de WebSocket diferentes do padrão legado.

Configurações novas em Config.h:
- fluidnc_path: use "/" (legado) ou "/ws" (novo)
- fluidnc_alt_port: porta alternativa para fallback (ex.: 80)

Comportamento novo:
- Tenta conexão no path informado e faz fallback para o outro path.
- Se falhar, tenta também a porta alternativa.
- Suporta frames TEXT para status GRBL (algumas builds novas enviam status em texto).
- A biblioteca FluidNC_WebSocket agora é local (lib/FluidNC_WebSocket) para manter os patches.
