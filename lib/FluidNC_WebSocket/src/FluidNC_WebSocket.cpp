...
ESP32Timer ITimer(0);
static bool timerAttached = false;
...
const char *fluidnc_path_default = "/";
...
{
    return connect(fluidnc_host_default, fluidnc_port_default, fluidnc_path_default);
}

bool FluidNC_WS::connect(const char* host, uint16_t port)
{
    return connect(host, port, fluidnc_path_default);
}

bool FluidNC_WS::connect(const char* host, uint16_t port, const char* path)
{
...
bool isConnected_internal()
{
    ...
    while (true)
    {
        ...
        if (millis() - startTime > TIMER_TIMEOUT_INTERVAL_MS) {
            DEBUG_SERIAL.printf("[FluidNC_WS::isConnected_internal] Timed out.\n");
            break;
        }
    }
...
void cmd_internal( SafeString &command )
{
    ...
    while (true)
    {
        ...
        if (millis() - startTime > TIMER_TIMEOUT_INTERVAL_MS) {
            DEBUG_SERIAL.printf("[FluidNC_WS::cmd_internal] Timed out.\n");
            break;
        }
    }
}
...
void webSocketEvent(WStype_t type, uint8_t * payload, size_t length) {
    ...
    switch(type) {
        ...
        case WStype_CONNECTED:
            ...
            if (!_isConnected) {
                _isConnected = true;
                if (_activeID < 0) _activeID = 0;
            }
            break;
        case WStype_TEXT:
            //DEBUG_SERIAL.printf("[webSocketEvent] TXT RESPONSE: %s\n", payload);
            {
                char textBuf[MAX_LEN];
                size_t copyLen = length < (MAX_LEN - 1) ? length : (MAX_LEN - 1);
                memcpy(textBuf, payload, copyLen);
                textBuf[copyLen] = '\0';

                check_if_connected(textBuf);
                if (!_isConnected) {
                    _isConnected = true;
                    if (_activeID < 0) _activeID = 0;
                }

                // Some newer FluidNC builds send GRBL status as TEXT instead of BIN
                if (textBuf[0] == '<' || strstr(textBuf, "MPos:") || strstr(textBuf, "WCO:")) {
                    bool full = (myCMD == getGrblFullStat);
                    _getGrblState(full, textBuf, strlen(textBuf));
                    if (myCMD == getGrblStat || myCMD == getGrblFullStat) {
                        isProcessingDone = true;
                        myCMD = None;
                    }
                } else if (myCMD == Raw) {
                    _executeFluidNC_Command(textBuf, strlen(textBuf));
                    isProcessingDone = true;
                    myCMD = None;
                }
            }
